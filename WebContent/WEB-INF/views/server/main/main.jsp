...
<html>
  <head>
     ...
     <script type="text/javascript">
       itour.main.setCookie('<%=SessionUtils.SESSION_USER%>','<%=session_user%>');
     </script>  
  </head>
		...
		<div id="tree-box" class="easyui-accordion" data-options="fit:true,border:false">
			<c:forEach var="item" items="${menuList}">
				<div title="${item.text}">
					...
					<a class="menu-item" onclick="javascript:dirurl(this,'${basePath}${node.url}')"  href="javascript:void(0)">${node.text}</a>
					</c:forEach>
				</div>
			</c:forEach>
		</div>
		   <script type="text/javascript">
          var curuser =itour.main.getCookie('<%=SessionUtils.SESSION_USER%>');
		...
		<div class="easyui-tabs" id="tab-box" data-options="fit:true,border:false">
		   <!--  <iframe scrolling='auto' frameborder='0' style='width:100%; height:100%'>
			...
			<div title="Welcome" style="padding:20px;overflow:auto;"> 
                ...
                <pre style="font-size:1.5em;font-family: '微软雅黑';">欢迎进入主角旅行（繁体）中文版网站后台管理界面</pre>
                <table style="margin-top:20px;width:100%;height:auto;border:1px solid grey;overflow-y:scroll;" border="1" id="unDealedOrders">
                </table>
            </div>
		</div>	
	</div>
