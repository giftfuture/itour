						...
						if(resp.success||resp.success=="true"){
							...
							$(resp.data).each(function(idx,ex){
								if(ex.id && ex.routeId && ex.orderName){
									...
									"<td style='text-align:left;'><a target='_blank' href="+basePath+"travelOrder/toQuote1/"+ex.id+"/"+ex.routeId+">"+ex.orderName+"</a></td><td style='text-align:center;'>"+ex.createTime+"</td><td style='text-align:center;'><a  class='easyui-linkbutton'  data-options='iconCls:\"icon-search\"' onclick='javascript:itour.main.unDealedOrdersWin(\""+ex.id+"\")' href='javascript:void(0)'>详情</a></td></tr>");
								}
							});
						}else{
						...
						if(resp.success||resp.success=="true"){
							...
							$(resp.data).each(function(idx,ex){
								//if(ex.id && ex.routeId && ex.orderName){
									...
									"<td style='text-align:center;'>"+ex.createTime+"</td><td style='text-align:center;'>"+ex.routeTitle+"</td><td style='text-align:center;'><a  class='easyui-linkbutton'  data-options='iconCls:\"icon-mini-edit\"' onclick='javascript:itour.main.unDealedDiarysWin(\""+ex.id+"\")' href='javascript:void(0)'>处理</a></td></tr>");
								//}
							});
						}else{
						...
						if(resp.success||resp.success=="true"){
							...
							$(resp.data).each(function(idx,ex){
								//if(ex.id && ex.routeId && ex.orderName){
									...
									"<td style='text-align:center;'>"+ex.routeName+"</td><td style='text-align:left;'>"+ex.content+"</td><td style='text-align:center;'><a   class='easyui-linkbutton'  data-options='iconCls:\"icon-mini-edit\"' onclick='javascript:itour.main.unCheckedMsgsWin(\""+ex.id+"\")' href='javascript:void(0)'>处理</a></td></tr>");
								//}
							});
						}else{
